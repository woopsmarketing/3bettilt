 ...
 *
 * WP-H1's own batch gate — same reasoning as `h2.test.ts`'s sibling file (and
 * `registry/glossary/j1.test.ts` before it): this workspace's `vitest.config.ts` registers
 * no MDX transform for the `fishtilt` project, so importing a real `.mdx` file (even one
 * this app already builds and ships, like `poker-range.mdx`) fails at Vite's import-analysis
 * step before any JSX runs. That is a pre-existing gap in a shared root file, out of this
 * batch's file boundary, not something to paper over with a mock that would prove nothing
 * real.
 *
 ...
 * permanent assignment from WP-G4/the content plan §7 "H1", not a fact about the site's
...
const MDX_MAP_SOURCE = readFileSync(
  fileURLToPath(new URL('../../learn/h1.ts', import.meta.url)),
  'utf8',
);

/** The five slugs `docs/FISHTILT_CONTENT_PLAN.md` §7 "H1" assigns to this batch. Fixed —
 * see the module doc on why this is a safe literal fixture rather than a derived one.
 * Lesson 02's slug is `poker-hand-rankings` (its id stays `hand-rankings`, ruling 11). */
const OWNED_SLUGS = [
  'holdem-basics',
  'poker-hand-rankings',
  'starting-hands',
...
const MEASURED: ReadonlyMap<string, ContentMeasurement> = new Map(
  LEARN_H1_RECORDS.map((record) => [record.id, measureContent(mdxSourceFor(record))]),
);
...
describe('learn batch H1 — registry', () => {
  it('registers exactly the five owned slugs, once each, and never lesson 06', () => {
    const slugs = LEARN_H1_RECORDS.map((record) => record.slug);
  ...
  it('every owned record is PUBLISHED, indexable, with a positive readMinutes', () => {
    for (const record of LEARN_H1_RECORDS) {
  ...
  it('carries the outbound links a learn record needs (build spec §37 / plan §5)', () => {
    for (const record of LEARN_H1_RECORDS) {
      ...
      // Lesson 01 (holdem-basics) is the curriculum's entry point and carries no
      // prerequisite by design; every other H1 lesson (order 2-5) requires one.
      if (record.id !== 'holdem-basics') {
        expect(record.prerequisites.length, `${record.id}: prerequisites`).toBeGreaterThanOrEqual(
          1,
        );
      }
    }
  ...
  it('lesson 02 keeps id "hand-rankings" but slug "poker-hand-rankings" (ruling 11)', () => {
    const record = LEARN_H1_RECORDS.find((entry) => entry.slug === 'poker-hand-rankings');
    expect(record).toBeDefined();
    expect(record?.id).toBe('hand-rankings');
  });
});

describe('learn batch H1 — MDX map registration', () => {
  ...
  it('the map carries no other slug than the five this batch owns', () => {
    const exportBlock = MDX_MAP_SOURCE.slice(MDX_MAP_SOURCE.indexOf('LEARN_H1_MDX'));
...
describe('learn batch H1 — MDX prose (content.test.ts equivalents, scoped to this batch)', () => {
  ...
  it('never asserts a profitability/strength verdict on raw strength or equity numbers (rule 0.10)', () => {
    for (const record of LEARN_H1_RECORDS) {
      const source = mdxSourceFor(record);
