...
| D-S3-23 | **기본 로케일 무접두(2026-09-17, owner directive).** 한국어(`DEFAULT_LOCALE`)는 URL 접두사 없음(`/`, `/learn/pot-odds`), 비기본 로케일만 `/<code>/…`(미래 `/en/…`). 페이지는 route group `src/app/(default-locale)/…`(디렉토리 = 공개 URL, rewrite·middleware 없음, 전부 정적). `/`는 한국어 홈 200. 기존 `/ko/*` 142개(141 indexable + `/search`)는 `lib/legacyLocaleRedirects.ts`의 **동결 목록**으로 1:1 308 redirect(패턴 없음 → `/ko/does-not-exist`는 그대로 404). root canonical 문자열은 Next가 렌더하는 bare origin `https://3bettilt.com`(= `/`와 동일 URL)으로 sitemap·hreflang·JSON-LD 모두 통일. 한국어에 `/ko` 접두를 다시 도입하지 않는다. **D-S3-01(`[locale]` 세그먼트)·D-S3-03(`/`→`/ko`)·D-S3-04(`/ko` canonical)를 대체**, D-S3-06은 URL만 무접두로 바뀌고 유지 | owner 지시(`prompt` 2026-09-17). rewrite 방식은 공개 URL과 실제 프리렌더 경로가 달라지는 취약 구조라 기각. 대가: 미래 `/en`은 `src/app/[locale]/…`에 얇은 route 파일이 필요(배열 원소 추가만으로는 부족) — `docs/3BETTILT_MULTILINGUAL_ARCHITECTURE.md` §2 |
