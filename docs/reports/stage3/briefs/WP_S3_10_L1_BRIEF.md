You are a content agent for **WP-S3-10 LEARN CONTENT ENRICHMENT, batch L1** of the 3BetTilt Stage 3 redesign (repo `/Users/woops/projects/GTO-SELF`, app `apps/fishtilt`).
...
## Your lessons (batch L1): holdem-basics, poker-hand-rankings, starting-hands, starting-hand-ranking, hand-matrix
Registry file you own: `apps/fishtilt/src/content/registry/learn/h1.ts` (+ its test).
...
- `apps/fishtilt/content/learn/{holdem-basics.mdx,poker-hand-rankings.mdx,starting-hands.mdx,starting-hand-ranking.mdx,hand-matrix.mdx}`
- `apps/fishtilt/src/content/registry/learn/h1.ts` and its `.test.ts`
- optional new test file `apps/fishtilt/src/content/learn-L1.claims.test.ts`
- `docs/reports/stage3/handoff/WP_S3_10_L1_HANDOFF.md`
...
- A build + screenshot of 2 of your lessons at 1440x900 and 390x844 (dark + light) via build-lock + shoot.mjs into `artifacts/3bettilt-stage3-visual-qa/wp10-L1/` — look at them (figures legible on mobile, no overflow). If the build is broken by another agent's file, note it and retry once later; don't block forever.
