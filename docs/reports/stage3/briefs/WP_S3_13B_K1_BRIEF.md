# WP-S3-13b HANDS CONTENT — batch K1 brief
...
## Your hands (batch K1): aa, kk, qq, jj, tt
## File boundary
- `apps/fishtilt/content/hands/{aa.mdx,kk.mdx,qq.mdx,jj.mdx,tt.mdx}`
- `apps/fishtilt/src/content/registry/hands/k1.ts` + `k1.test.ts`
- `docs/reports/stage3/handoff/WP_S3_13B_K1_HANDOFF.md`
...
- Every number via `<Fact>` or pinned by a test in your `k1.test.ts` computed from the engine; close the NUMBER-RISK
...
- Build + screenshots of 2 of your hands (1440x900 + 390x844, dark + light) via build-lock + shoot.mjs into
  `artifacts/3bettilt-stage3-visual-qa/wp13b-K1/`; LOOK at them (no duplicated facts, FAQ renders, no overflow).
