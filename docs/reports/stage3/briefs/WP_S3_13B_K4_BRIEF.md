# WP-S3-13b HANDS CONTENT — batch K4 brief
...
## Your hands (batch K4): kqs, kjs, qjs, jts, t9s
## File boundary
- `apps/fishtilt/content/hands/{kqs.mdx,kjs.mdx,qjs.mdx,jts.mdx,t9s.mdx}`
- `apps/fishtilt/src/content/registry/hands/k4.ts` + `k4.test.ts`
- `docs/reports/stage3/handoff/WP_S3_13B_K4_HANDOFF.md`
...
- Every number via `<Fact>` or pinned by a test in your `k4.test.ts` computed from the engine; close the NUMBER-RISK
...
- Build + screenshots of 2 of your hands (1440x900 + 390x844, dark + light) via build-lock + shoot.mjs into
  `artifacts/3bettilt-stage3-visual-qa/wp13b-K4/`; LOOK at them (no duplicated facts, FAQ renders, no overflow).
