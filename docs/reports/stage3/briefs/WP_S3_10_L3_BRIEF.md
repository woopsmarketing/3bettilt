You are a content agent for **WP-S3-10 LEARN CONTENT ENRICHMENT, batch L3** of the 3BetTilt Stage 3 redesign (repo `/Users/woops/projects/GTO-SELF`, app `apps/fishtilt`).
...
## Your lessons (batch L3): flop-turn-river, three-bet, equity, pot-odds, outs
Registry file you own: `apps/fishtilt/src/content/registry/learn/h3.ts` (+ its test).
...
- `apps/fishtilt/content/learn/{flop-turn-river.mdx,three-bet.mdx,equity.mdx,pot-odds.mdx,outs.mdx}`
- `apps/fishtilt/src/content/registry/learn/h3.ts` and its `.test.ts`
- optional new test file `apps/fishtilt/src/content/learn-L3.claims.test.ts`
- `docs/reports/stage3/handoff/WP_S3_10_L3_HANDOFF.md`
...
- A build + screenshot of 2 of your lessons at 1440x900 and 390x844 (dark + light) via build-lock + shoot.mjs into `artifacts/3bettilt-stage3-visual-qa/wp10-L3/` — look at them (figures legible on mobile, no overflow). If the build is broken by another agent's file, note it and retry once later; don't block forever.
