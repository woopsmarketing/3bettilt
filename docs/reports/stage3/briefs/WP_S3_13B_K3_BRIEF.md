# WP-S3-13b HANDS CONTENT — batch K3 brief
...
## Your hands (batch K3): aks, ako, aqs, aqo, ajs
## File boundary
- `apps/fishtilt/content/hands/{aks.mdx,ako.mdx,aqs.mdx,aqo.mdx,ajs.mdx}`
- `apps/fishtilt/src/content/registry/hands/k3.ts` + `k3.test.ts`
- `docs/reports/stage3/handoff/WP_S3_13B_K3_HANDOFF.md`
...
- Every number via `<Fact>` or pinned by a test in your `k3.test.ts` computed from the engine; close the NUMBER-RISK
...
- Build + screenshots of 2 of your hands (1440x900 + 390x844, dark + light) via build-lock + shoot.mjs into
  `artifacts/3bettilt-stage3-visual-qa/wp13b-K3/`; LOOK at them (no duplicated facts, FAQ renders, no overflow).
