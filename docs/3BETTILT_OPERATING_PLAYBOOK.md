...
1. **Search Console → 검색 실적**, 최근 28일 vs 이전 28일. 필터: 없음(사이트 전체 — D-S3-23 이후 한국어 URL에 접두사가 없다). 보는 것: 노출 상승 질의, CTR 낮은 질의(노출 있는데 클릭 없음), 순위 8–20위 질의(한 걸음이면 1페이지).
...
4. **Pages(색인 생성) 보고서**: "색인 생성됨" 수가 sitemap `<loc>` 수와 같은가. 새로 생긴 "제외됨" 사유가 있으면 원인 확인(중복·canonical 불일치·noindex — `/search` 하나만 정상).
...
- **본문 링크**: `[텍스트](/learn/pot-odds)`처럼 **로케일 없는** 경로로 쓴다. 로케일은 렌더 시 `localiseHref`가 적용한다(한국어는 무접두, D-S3-23). `/ko/...`로 쓰면 빌드가 실패한다. `claims.test.ts`가 존재하지 않는 경로를 잡는다.
...
| 404 유입 | GSC Pages "찾을 수 없음(404)" | 외부 링크의 오타·삭제된 slug, 또는 목록에 없던 `/ko/...` 주소 | 기존 `/ko/*` 142개는 1:1 redirect(D-S3-23). 그 밖의 주소는 404 유지 — 실제 유입이 확인되면 그 근거로 개정 요청 |
