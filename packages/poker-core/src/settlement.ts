 ...
 * unawarded pot in a single command.
 */
...
/** Total. True once the board reached three cards. The rake's no-flop-no-drop test. */
...
/**
 * Total. Per-winner rake attribution for one pot, in the same order as `shares`.
 * The event stores only the pot's total rake; this is how the reducer reproduces each
 * seat's `rakePaid` so the standing per-seat chip identity holds.
 */
...
function planFor(
  ...
  const records: PotAwardRecord[] = targets.map((target, index) => {
    ...
    const netAmount = Money.sub(target.pot.amount, rake);
    return {
  ...
  return ok({ records, totalRake: rakeResult.rake, reason });
}
...
export function planAwards(
  ...
  return planFor(state, awards, 'SHOWDOWN');
}
...
export function autoAwardUncontested(state: HandState): EngineResult<SettlementPlan> {
  ...
  return planFor(state, awards, 'ALL_FOLDED');
}
...
/** Throws. At COMPLETE, `sum(seat net) + totalRake === ZERO`. */
export function assertSettlementBalances(state: HandState): void {
  ...
  invariant(
    Money.add(net, state.totalRake) === Money.ZERO,
    `settlement does not balance: net ${net} + rake ${state.totalRake} != 0`,
  );
}
